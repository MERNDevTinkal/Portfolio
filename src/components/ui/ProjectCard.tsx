...
export function ProjectCard({ project, index }: ProjectCardProps) {
  ...
  return (
    ...
    >
      <Card className="h-full flex flex-col overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 bg-card border border-border/70 hover:border-primary/50" style={{ transform: 'translateZ(0)' }}>
        <CardHeader className="pb-3 pt-5 px-4 sm:px-6">
          <CardTitle className="text-lg sm:text-xl md:text-2xl font-headline text-primary line-clamp-2">{project.title}</CardTitle>
        </CardHeader>
        ...
        <CardFooter className="mt-auto pt-4 pb-5 px-5">
          {!isMounted ? (
            ...
            <>
              {hasValidLiveDemo ? (
                <Button asChild className="w-full transform hover:scale-105">
                ...
                <Button className="w-full transform hover:scale-105" onClick={handleLiveDemoClick}>
                  <span>
